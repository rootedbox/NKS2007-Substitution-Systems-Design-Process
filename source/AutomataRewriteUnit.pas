...
type
  ...
  public
    Points : array[1..SpaceSize, 1..SpaceSize] of Byte;
    constructor Create;
    function GetPoint(x,y : Integer) : Byte;
    procedure SetPoint(x,y, PointValue : Byte);
  end;
...
begin
  for XIndex := 1 to SpaceSize do
    for YIndex := 1 to SpaceSize do
      Space2.SetPoint(XIndex, YIndex, Space.GetPoint(XIndex, YIndex));

  for XIndex := 1 to SpaceSize do
    for YIndex := 1 to SpaceSize do
    begin
      StateIndex := 0;
      StateIndex := StateIndex or  Space2.GetPoint(XIndex - 1, YIndex - 1);
      StateIndex := StateIndex or (Space2.GetPoint(XIndex    , YIndex - 1) * 2);
      StateIndex := StateIndex or (Space2.GetPoint(XIndex + 1, YIndex - 1) * 4);

      Space.SetPoint(XIndex, YIndex, PossibleStates.GetResult(StateIndex));
    end;
...
begin
  ...
  try
    ...
    for ySpaceIndex := 1 to SpaceSize do
      ...
      begin
        Space.Points[xSpaceIndex, ySpaceIndex] := StrToInt(Text.Strings[TextIndex]);
        Inc(TextIndex);
      end;
...
begin
  ...
  try
    ...
    for ySpaceIndex := 1 to SpaceSize do
      for xSpaceIndex := 1 to SpaceSize do
      begin
        Text.Add(IntToStr(Space.Points[xSpaceIndex, ySpaceIndex]));
      end;
...
function TSpace.GetPoint(x, y : Integer): Byte;
begin
  x := AdjustCordinate(x);
  y := AdjustCordinate(y);
...
begin
  for ySpaceIndex := 1 to SpaceSize do
    for xSpaceIndex := 1 to SpaceSize do
      Points[xSpaceIndex, ySpaceIndex] := 0;
end;

procedure TSpace.SetPoint(x, y, PointValue: Byte);
begin
  x := AdjustCordinate(x);
  y := AdjustCordinate(y);

  Points[x, y] := PointValue;
end;
...
var
  ...
  StateIndex : Integer;
begin
  for StateIndex := 1 to 8 do //Trunc(Power(2, StatePointCount)) do
  begin
